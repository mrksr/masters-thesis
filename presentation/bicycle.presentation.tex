...
\begin{frame}[label=gp_non_parametric_models]
    ...
    \begin{overlayarea}{\textwidth}{8\baselineskip}
        \only<1> {
            \begin{block}{Data}
                \begin{itemize}
                    \item Observe a \structure{noisy} data set $\D = \Set{\left( \mat{x_i}, \mat{y_i} \right)}$
        ...
        \only<2> {
            \begin{block}{Non-Bayesian Parametric Models}
                \begin{itemize}
                    ...
                    \item Everything is \structure{deterministic}
                \end{itemize}
            \end{block}
        }
        \only<3-4> {
            \begin{block}{Bayesian Parametric Models}
                \begin{itemize}
                    \item Assume \structure{structure} about $f$, such as $f(\mat{x}) = \mat{W}\mat{x}$
                    \item Find a \structure{distribution} $\Prob{\mat{W} \given \D}$ of plausible parameters
        ...
        \only<5-> {
            \begin{block}{Bayesian Non-Parametric Models}
                \begin{itemize}
                    ...
                    \item Find a \structure{distribution} $\Prob{f \given \D}$ over functions
                    \item We can consider all \structure{functions} at once
                \end{itemize}
            \end{block}
        }
    \end{overlayarea}
...
\begin{frame}[label=gp_definition]
    ...
    \begin{definition}[Gaussian Process]
        A \structure{Gaussian Process (GP)} is a collection of random variables $\Set{\rv{F_x} \with x \in X}$, any finite subset of which has a joint Gaussian distribution.
    \end{definition}
    \begin{itemize}
    ...
    \begin{block}{Mean and Kernel Functions}
        A GP is \structure{completely determined} by
...
\begin{frame}[label=gp_hyperparameters]
