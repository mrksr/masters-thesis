...
\begin{standaloneframe}
    \begin{tikzpicture}[stop jumping]
        ...
        \only<3->{
            \node[entity] (pso) at (0, 1.5) {PSO-P};
            \node[entity] (gp) at (0, -1.5) {GP Models};

            \only<4->{
                \draw[edge, directed, thick]
                    ([yshift=-3pt]pso.east) -- ($([yshift=-3pt]pso) + (2, 0)$)
                    |- node[pos=0.25, left] {$a_{t}$\\\vdots\\$a_{t+T-1}$} ([yshift=3pt]gp.east);

                \draw[edge, inverse directed, thick]
                    ([yshift=-3pt]pso.west) -- ($([yshift=-3pt]pso) + (-2, 0)$)
                    |-
                    node[pos=0.25, right] {$s_{t+1}$\\$\vdots$\\$s_{t+T}$}
                    % node[pos=0.25, right] {$\Rwd(s_{t+1})$\\$\vdots$\\$\Rwd(s_{t+T+1})$}
                    ([yshift=3pt]gp.west);
