...
\begin{standaloneframe}
    \begin{tikzpicture}[stop jumping]
        ...
        \begin{axis} [
            ...
            \addplot[mark=x, only marks] table[x=X, y=y] {\plotdatapath/spgp_data.dat};

            \only<2-> {
                \addplot[thin, tumblue, name path=c, restrict x to domain=-5:5] table[x=X, y expr=\thisrow{y} + 2 * \thisrow{y_std}] {\plotdatapath/spgp_full.dat};
                \addplot[thin, tumblue, name path=d, restrict x to domain=-5:5] table[x=X, y expr=\thisrow{y} - 2 * \thisrow{y_std}] {\plotdatapath/spgp_full.dat};
                \addplot[tumblue!15] fill between[of=c and d, on layer={axis background}];
                \addplot[very thick, tumblue] table[x=X, y=y] {\plotdatapath/spgp_full.dat};
            }
        \end{axis}
