...
\begin{table}[t]
    ...
    \begin{tabularx}{\tablewidth}{ccXc}
        ...
        \addlinespace
        GP models & $\mat{N}$ & Number of training points & $\Set{60000, 70000}$ \\
        & $\mat{M}$ & Number of pseudo-inputs & $\Set{20, 30, 40, 50, 75, 100, 150}$ \\
        & $\K$ & Kernel & RBF \\
        \addlinespace
    ...
    \begin{algorithmic}[1]
        \For{$i \gets 1, I$}
            ...
            \ForAll{$(N, M) \in \mat{N} \times \mat{M}$}
                \State Train transition model $f^{(N, M)}$ on $\D_i$
            \EndFor

            \For{$s \gets 1, S$}
                \State Sample start state $\mat{s^{(s)}_0} \sim \Prob{\mat{s_0}}$
