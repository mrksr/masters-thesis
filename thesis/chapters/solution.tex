...
For every such time step, the accumulated probability $\ms{\PFallen}_t$ of already having fallen down at some time before or at time step~$t$ increases.
...
Having calculated the terminal probabilities $\ms{\PGoal_t}$ and $\ms{\PFallen_t}$ it is given by
\begin{align}
    ...
    \begin{split}
        ...
        &= \sum_{t=0}^T \gamma^t \Moment*{\E}{\RwdBicycle(\mat{s_t}) \given f, \mat{s_0}, \mat{a_0}, \dots, \mat{a_{T-1}}} \\
        &= \sum_{t=0}^T \gamma^t \bigg( 2 \cdot \ms{\PGoal}_t + (1 - \ms{\PGoal}_t)(1 - \ms{\PFallen}_t) \\
...
\end{figure}
In the action value function, this is reflected by the accumulative probabilities $\PGoal$ and $\PFallen$.
...
The multi-step uncertainties without the truncation of Gaussians therefore overestimate the terminal probabilities $\PGoal_t$ and $\PFallen_t$.
