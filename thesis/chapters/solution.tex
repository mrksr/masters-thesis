...
\todo{add note about comparison section}
...
In order to simulate a deterministic transition model, this first approach for long-term predictions discards this believe and only predicts a single successive state.
...
\begin{figure}[t]
    \centering
    \includegraphics[height=.55\textheight]{map_predictions.png}
...
The following section introduces how the reward function can be extended to believes about states and uses the uncertainty predicted by the transition model during planning without propagating it through multiple time steps.
...
With these probabilities describing the overall believe that the predictive trajectory has not already ended, the expected reward for every state in the trajectory can be calculated.
...
\begin{figure}[t]
    \centering
    \includegraphics[height=.55\textheight]{ms_predictions.png}
...
After about \todo{Fix this number} ten time steps, the uncertainties about $\omega$ grow and two standard deviations cover most of the value range.
...
\begin{figure}[t]
    \centering
    \includegraphics[height=.55\textheight]{tg_predictions.png}
...
\todo{this will be a section in this chapter}
