...
\begin{figure}[tp]
    ...
    \begin{subfigure}{\subfigurewidth}
        \includegraphics[height=.35\textheight]{data_set_length_histogram.png}
        \caption{Length of Episodes}
        \label{fig:data_set_properties:episode_length}
    \end{subfigure}
    \begin{subfigure}{\subfigurewidth}
        \includegraphics[height=.35\textheight]{data_set_omega_plot.png}
...
\begin{figure}[tp]
    \centering
    \includegraphics{data_set_positions.png}
...
\begin{figure}[t]
    \centering
    \missingfigure[figheight=.35\textheight]{Comparison of linear and saturating reward.}
...
\begin{figure}[t]
    \centering
    \includegraphics[height=.55\textheight]{predictions_1.png}
...
Given a time horizon $T$, a starting state $\mat{s_0}$ and a vector $(\mat{a_0}, \dots, \mat{a_{T-1}})$ of actions, this function can be used to calculate the intermediate states $\map{\mat{s_1}}$ to $\map{\mat{s_T}}$ using dynamic programming. \Cref{fig:map_predictions} compares these iterated MAP-predictions with the correct evolution of the bicycle system.
...
\todo{which plot do we use?} \Cref{fig:map_results} shows the percentage of trajectories which reach the goal plotted against the number of pseudo inputs.
...
In order to correctly calculate the probability that the trajectory has already ended at time step $t+1$, the probabilities of falling down or reaching the goal at any of the previous time steps have to be considered.
Let
\begin{align}
...
\begin{align}
    ...
    &\qquad\qquad\qquad\qquad\sqrt{2\pi\sigma_{\text{angle}}^2} \Gaussian{\pi\tran \mat{\mu_t} \given 0, \sigma_{\text{angle}}^2 + \pi\tran \mat{\Sigma_t}\pi}.
\end{align}
...
Similarly, for deterministic states, both $\Prob{\PGoal_t}$ and $\Prob{\PFallen_t}$ are either zero or one.
...
\begin{figure}[tp]
    \centering
    \includegraphics[height=.35\textheight]{os_goal_percentage.png}
...
\begin{figure}[p]
    \centering
    \includegraphics{positions_3.png}
...
Small errors in the single predictions accumulate and after multiple time steps very close to the correct trajectory, predicted trajectories tend to diverge from the truth very rapidly.
...
This section introduces an approximation of this propagation via linearization of the transition model around the predictive mean as presented in \cite{ko_gp-bayesfilters:_2009, deisenroth_gaussian_2015}.
...
Using the MAP-predictions, $\mat{\Delta_t}$ was assumed to be one deterministic value, while for using the one-step uncertainties, the complete Gaussian distribution $\rv{\Delta_t} \sim \Gaussian{\mat{\mu_\Delta}, \mat{\Sigma_\Delta}}$ was used to compute the distribution of the successive state.
Since $\mat{s_t}$ was assumed constant, the distribution of $\os{\rv{s_{t+1}}}$ was given by
\begin{align}
    \os{\rv{s_{t+1}}} &= \mat{s_t} + \rv{\Delta_t}, \\
    \os{\rv{s_{t+1}}} &\sim \Gaussian{\mat{s_t} + \mat{\mu_\Delta}, \mat{\Sigma_\Delta}}.
\end{align}
In both cases, the state was again assumed to be deterministic and at $\Moment{\E}{\rv{s_{t+1}}}$ in order to derive the distribution of the next state $\rv{s_{t+2}}$, preventing the accumulation of uncertainties.
...
\end{align}
that is, by marginalizing both the believe over the current state $\rv{s_t}$ and all plausible transition models $f$.
...
Since $\rv{s_{t+1}}$ is the sum of $\rv{s_t}$ and $\rv{\Delta_t}$, a Gaussian approximation of its distribution is given by
\begin{align}
...
\end{align}
where $\Moment{\cov}{\rv{s_t}, \rv{\Delta_t}}$ denotes the covariance between the current state and the state change.
...
Let $\alpha \sim \Gaussian{\mu, \sigma^2}$ denote an angle with a Gaussian distribution.
Using Euler's identity, it can be shown that the expected values of its sine and cosine are given by
\begin{align}
    \Moment{\E}{\sin \alpha} &= \exp\!\left( -\frac{\sigma^2}{2} \right)\sin\mu, \\
    \Moment{\E}{\cos \alpha} &= \exp\!\left( -\frac{\sigma^2}{2} \right)\cos\mu,
\end{align}
respectively \cite{deisenroth_efficient_2010}.
The variances of the two random variables can be calculated as
\begin{align}
    \Moment{\var}{\sin \alpha} &= \frac{1}{2} - \frac{1}{2} \exp\!\left( -2 \sigma^2 \right) \cos(2\mu) - \exp(- \sigma^2) \sin^2\mu, \\
    \Moment{\var}{\cos \alpha} &= \frac{1}{2} + \frac{1}{2} \exp\!\left( -2 \sigma^2 \right) \cos(2\mu) - \exp(- \sigma^2) \cos^2\mu.
\end{align}
...
The second step in the algorithm is to calculate $\Prob{\rv{\Delta_t}}$ from the augmented state distributrion.
...
Using the different Gaussian process models, the derivative of the predictive mean function can be calculated explicitly.
It is a matrix
\begin{align}
...
The Gaussian processes used in this thesis only make the prior assumption that the transition dynamics of the bicycle benchmark are a smooth function.
They are therefore trained using a squared exponential kernel.
...
The believe about the successive state is a Gaussian distribution characterized by
\begin{align}
    \rv{s_{t+1}} &\sim \Gaussian{\mat{\mu_{t+1}}, \mat{\Sigma_{t+1}}} \text{, where} \\
    \mat{\mu_{t+1}} &= \mat{\mu_t} + \mat{\mu_\Delta} \\
...
\begin{figure}[t]
    \centering
    \includegraphics[height=.55\textheight]{predictions_3.png}
...
In contrast to the one-step uncertainties presented shown in \cref{fig:os_uncertainties}, the state distributions obtained using linearization quite accurately estimate the predictive error.
The uncertainties reach the same order of magnitude as the dynamic range of their variable after a few time steps and become visible in the plot after a few time steps.
After about \todo{Fix this number} ten time steps, the uncertainties about $\omega$ explode and two standard deviations cover most of the value range.
At this point, the model's predictions can be interpreted as almost complete uncertainty about the correct state.
At the same time however, the predicted mean is no longer close to the correct trajectory.
...
\begin{figure}[t]
    \centering
    \includegraphics[height=.55\textheight]{predictions_0.png}
...
The probabilities $\Prob{\tg{\rv{s_{t+1}}} \in \Goal}$ and $\Prob{\tg{\rv{s_{t+1}}} \in \Fallen}$ therefore correctly represents the probability of reaching the goal or falling exactly at time step $t+1$ respectively.
...
This effect is also visible ein \cref{fig:ms_successful_trajectory}, which shows all state variables over time for one successful trajectory.
...
If the bicycle is exactly upright, it is hard for the models to predict if the bicycle will fall towards the left or the right and therefore, which leads to a bimodal distribution.
...
As a baseline for comparison, PSO-P was to the action-value-function $\AVlu$ using a reward function $\RwdBicycle$ formulated using reward shaping.
...
The next chapter compares the different results in more detail and discusses their interpretability concerning other problems in reinforcement learning.
Lastly, it presents possible improvements and further directions of research.
