...
Let $\alpha \sim \Gaussian{\mu, \sigma^2}$ be a normally distributed angle.
Then
\begin{align}
...
\end{align}
We assume $\Moment{\cov}{\sin \psi, \omega} = 0$ etc.

We would like to calculate
\begin{align}
    \Prob{\rv{\Delta_t}} = \iint \Prob{f(\rv{\tilde{s}_t}) \given \rv{\tilde{s}_t}} \Prob{\rv{\tilde{s}_t}} \diff f \diff \rv{\tilde{s}_t}
\end{align}
however, this is not exactly nice, see \cref{fig:linearization}.
Therefore, we approximate it with a Gaussian.
Assuming we know this, then $\rv{s_{t+1}} \coloneqq \rv{s_t} + \rv{\Delta_t}$ and
\begin{align}
...
Well, we just assume that
\begin{align}
    \rv{\mu_\Delta} = \Moment{\E}{f(\rv{\tilde{\mu}_t})}
\end{align}
\begin{align}
    \rv{\mu_\Delta}^d = \Moment{\E}{f_d(\rv{\tilde{\mu}_t})} = \K_a( \rv{\tilde{\mu}_t}, \mat{\tilde{X}})\mat{\beta_a}
\end{align}
an explicitly linearize the posterior GP mean function around $\rv{\tilde{\mu}_t}$.
This linearization is given by
\begin{align}
    V &= \frac{\partial}{\partial \rv{\tilde{\mu}_t}} \rv{\mu_\Delta} = \frac{\partial \K(\rv{\tilde{\mu}_t}, \mat{\tilde{X}})}{\partial \rv{\tilde{\mu}_t}}\mat{\beta}
\end{align}
We calculate the predictive variances as
\begin{alignat}{2}
    \mat{\Sigma_\Delta} &= \Moment{\var}{V \rv{\tilde{s}_t}} &&+ \mat{\tilde{\Sigma}_f} \\
    &= V \mat{\tilde{\Sigma}_t} V\tran &&+ \diag\left(
        \Moment*{\var}{f_1(\rv{\tilde{\mu}_t})},
        \cdots,
        \Moment*{\var}{f_D(\rv{\tilde{\mu}_t})}
    \right)
\end{alignat}
using standard results about Gaussian transformations.
Also,
\begin{align}
    \Moment{\cov}{\rv{s_t}, \rv{\Delta_t}} = \Moment{\cov}{\rv{s_t}, V\rv{s_t}} = \Moment{\cov}{\rv{s_t}, \rv{s_t}}V\tran = \mat{\tilde{\Sigma}_t}V\tran
\end{align}
and equivalently, $\Moment{\cov}{\rv{\Delta_t}, \rv{s_t}} = V \mat{\tilde{\Sigma}_t}$.
...
\begin{align}
    \rv{s_{t+1}} &\sim \Gaussian{\mat{\mu_{t+1}}, \mat{\Sigma_{t+1}}} \text{, where} \\
    \mat{\mu_{t+1}} &= \mat{\mu_t} + \mat{\mu_\Delta} \\
    &= \mat{\mu_t} + \Moment{\E}{f(\rv{\tilde{\mu}_t})} \\
    \mat{\Sigma_{t+1}} &= \mat{\Sigma_t} + \mat{\Sigma_\Delta} + \Moment{\cov}{\rv{s_t}, \rv{\Delta_t}} + \Moment{\cov}{\rv{\Delta_t}, \rv{s_t}} \\
    &= \mat{\Sigma_t} + V\mat{\tilde{\Sigma}_t}V\tran + \mat{\tilde{\Sigma}_f} + \mat{\tilde{\Sigma}_t}V\tran + V\mat{\tilde{\Sigma}_t}
\end{align}
