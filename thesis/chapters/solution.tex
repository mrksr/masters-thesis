...
In their experiments, \citeauthor{randlov_learning_1998} \cite{randlov_learning_1998} chose a time discretization of 0.01 seconds.
...
\begin{figure}[t]
    ...
    \caption[Data set properties]{
        ...
        The last value of $\omega$ is in the valid range of values, since the last transition which results in a terminal state is not part of the training set.
    }
...
\label{sub:gp_models}
The Gaussian process models for the transition dynamics are trained using data sets of the form $\D = \Set{(\mat{s_i}, \mat{a_i}, \mat{s_i^\prime}) \in \Es \times \Ah \times \Es \with i \in [N]}$ of pairs of states and actions and their corresponding following state.
The following states $\mat{s_i^\prime} = \DynBicycle(\mat{s_i}, \mat{a_i})$ are obtained form the transition dynamics.
...
The most basic and uninformed reward function possible assigns positive reward for reaching the goal, a punishment (in the form of negative reward) for falling over and weights all other states equally somewhere between the two extremes.
...
\begin{figure}[t]
    ...
    \caption[Comparison of linear and saturating reward functions]{
        The blue quadratic saturating reward function is a Gaussian probability density which has been renormalized to a maximum value of one.
...
\Cref{fig:map_predictions} compares these iterated MAP-predictions with the correct evolution of the bicycle system in an example.
...
\begin{align}
    ...
    \begin{split}
        \map{\AVlu}_{\mat{s_0}}(\mat{a_0}, \dots, \mat{a_{T-1}}) &= \Moment*{\E}{\sum_{t=1}^T \gamma^t \RwdBicycle(\mat{s_t}) \given f, \mat{s_0}, \mat{a_0}, \dots, \mat{a_{T-1}}} \\
        &= \sum_{t=1}^T \gamma^t \Moment*{\E}{\RwdBicycle(\mat{s_t}) \given f, \mat{s_0}, \mat{a_0}, \dots, \mat{a_{T-1}}} \\
        &= \sum_{t=1}^T \gamma^t \RwdBicycle(\map{\mat{s_t}}),
    \end{split}
\end{align}
...
\begin{align}
    ...
    \begin{split}
        \os{\AVlu}_{\mat{s_0}}(\mat{a_0}, \dots, \mat{a_{T-1}}) &= \Moment*{\E}{\sum_{t=1}^T \gamma^t \RwdBicycle(\mat{s_t}) \given f, \mat{s_0}, \mat{a_0}, \dots, \mat{a_{T-1}}} \\
        &= \sum_{t=1}^T \gamma^t \Moment*{\E}{\RwdBicycle(\mat{s_t}) \given f, \mat{s_0}, \mat{a_0}, \dots, \mat{a_{T-1}}} \\
        &= \sum_{t=1}^T \gamma^t \bigg( 2 \cdot \Prob{\os{\PGoal}_t} + (1 - \Prob{\os{\PGoal}_t})(1 - \Prob{\os{\PFallen}_t}) \\
        &\qquad \sqrt{2\pi\sigma_{\text{angle}}^2} \Gaussian{\pi\tran \os{\mat{\mu_t}} \given 0, \sigma_{\text{angle}}^2 + \pi\tran \os{\mat{\Sigma_t}}\pi} \bigg).
    \end{split}
\end{align}

\subsection{Results using One-Step Uncertainties}
\begin{figure}[tp]
    \centering
...
\begin{figure}[tp]
    ...
    \caption[One step uncertainties]{
        ...
        The safety margin induced by these uncertainties therefore is also very small.
    }
...
\begin{figure}[p]
    \centering
    \includestandalonewithpath{figures/solution_ms_linearization}
    \caption[Uncertainty propagation using linearization]{
        This figure shows the propagation of uncertainties through a Gaussian process transition model (upper right panel).
...
\begin{align}
    ...
    \begin{split}
        \ms{\AVlu}_{\mat{s_0}}(\mat{a_0}, \dots, \mat{a_{T-1}}) &= \Moment*{\E}{\sum_{t=1}^T \gamma^t \RwdBicycle(\mat{s_t}) \given f, \mat{s_0}, \mat{a_0}, \dots, \mat{a_{T-1}}} \\
        &= \sum_{t=1}^T \gamma^t \bigg( 2 \cdot \Prob{\ms{\PGoal}_t} + (1 - \Prob{\ms{\PGoal}_t})(1 - \Prob{\ms{\PFallen}_t}) \\
...
\end{figure}
Using the intermediate state distributions $\Prob{\ms{\rv{s_t}}}$ directly when predicting successive states is a good approximation in most cases but is not correct when the terminal probabilities $\Prob{\rv{s_t} \in \Fallen}$ or $\Prob{\rv{s_t} \in \Goal}$ are large.
...
The transition model $f$ is however trained using only non-terminal state transitions, that is, state transitions where neither the prior nor the posterior state is terminal.
This implies the assumption that the transition model only operates on non-terminal states, similar to transition dynamics $\Dyn$ in general only being defined on $\Es$, the set of non terminal states.
The attractor property of terminal states has to be modelled outside of the transition models.
...
\subsection{Results using Multi-Step Uncertainties}
\begin{figure}[tp]
    \centering
...
The policy alternates between steering heavily to the left or to the right which results in the wavy movement.
...
\begin{align}
    \MeanRwdBicycle(\mat{s_0}, \dots, \mat{s_T}) \coloneqq \frac{1}{T} \sum_{t=1}^T \RwdBicycle(\mat{s_t}).
\end{align}
...
Given a set $\mat{\tau}$ of trajectories, the mean mean reward is defined as
\begin{align}
    \MeanMeanRwdBicycle(\mat{\tau}) \coloneqq \frac{1}{\abs{\mat{\tau}}} \sum_{i=1}^{\abs{\mat{\tau}}} \MeanRwdBicycle(\mat{\tau_i}).
\end{align}
...
The solution proposed in this thesis is to calculate accumulative probabilities $\Prob{\PGoal_t}$ and $\Prob{\PFallen_t}$ for the different termination conditions for every time step and to weight the expected reward accordingly.
