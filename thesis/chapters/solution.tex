...
\begin{align}
    \rv{\Delta_\psi} \sim \Gaussian{\pi^T\mat{\mu_t}, \pi^T \mat{\Sigma_t} \pi}.
\end{align}
...
\begin{align}
    ...
    &= c \int \Gaussian{\rv{\Delta_\psi} \given 0, \sigma_{\text{angle}}^2} \Gaussian{\rv{\Delta_\psi} \given \pi^T \mat{\mu_t}, \pi^T \mat{\Sigma_t} \pi} \diff \rv{\Delta_\psi} \\
    &= c \Gaussian{\pi^T \mat{\mu_t} \given 0, \sigma_{\text{angle}}^2 + \pi^T \mat{\Sigma_t} \pi}.
\end{align}
...
\begin{align}
    ...
    &= 2 \cdot \PGoal_t + (1 - \PGoal_t)(1 - \PFallen_t) \sqrt{2\pi\sigma_{\text{angle}}^2} \Gaussian{\pi^T \mat{\mu_t} \given 0, \sigma_{\text{angle}}^2 + \pi^T \mat{\Sigma_t}\pi}.
\end{align}
Note that for states with very low uncertainty, that is states for which $\pi^T \mat{\Sigma_t} \pi$ goes to zero, the expected value converges towards the original reward function.
...
\begin{align}
    ...
    \begin{split}
        ...
        &\qquad \sqrt{2\pi\sigma_{\text{angle}}^2} \Gaussian{\pi^T \os{\mat{\mu_t}} \given 0, \sigma_{\text{angle}}^2 + \pi^T \os{\mat{\Sigma_t}}\pi} \bigg).
    \end{split}
\end{align}
