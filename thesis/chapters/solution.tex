...
Given a time horizon $T$ and a starting state $\mat{s_0}$ and a vector $(\mat{a_0}, \dots, \mat{a_{T-1}})$ of actions, this function can be used to calculate the intermediate states $\map{\mat{s_1}}$ to $\map{\mat{s_T}}$ using dynamic programming. \Cref{fig:map_predictions} compares these iterated MAP-predictions with the correct evolution of the bicycle system.
...
The deterministic bicycle reward function hints towards the goal by increasing the reward if the bicycle's frame points towards the goal.
...
The following section introduces how the deterministic reward function can be extended to believes about states and uses the uncertainty predicted by the transition model in planning without propagating it through multiple time steps.
...
Instead of discarding them at every time step, the uncertrainties returned by the transition model are used to evaluate the expected reward.
These uncertainties are still not propagated however, as the algorithm for deriving long-term predictions essentially remains the same compared to the previous technique.

If the predictive state distributions $\Prob{\mat{s_1}}$ to $\Prob{\mat{s_T}}$ are no longer assumed to be delta-distributions (or deterministic states), the expected reward is no longer trivial to calculate.

\subsection{Probabilistic Bicycle Reward Function}
Given any reward function, we have to be able to solve
\begin{align}
    \Moment*{\E_{\rv{s}}}{\Rwd(\rv{s})} &= \int \Rwd(\rv{s}) \Prob{\rv{s}} \diff \rv{s} \\
    &= \int \Rwd(\rv{s}) \Gaussian{\rv{s} \given \mat{\mu_s}, \mat{\Sigma_s}} \diff \rv{s}
\end{align}

\begin{definition}[Probabilistic Bicycle Reward Function]
    Given the set $\Es^+$ of possible states of the bicycle benchmark, the \emph{probabilistic bicycle reward function} is defined as
    \begin{align}
        \RwdBicycleProb : \left\{
            \begin{aligned}
                \Es^+ &\to \R \\
                \rv{s} &\mapsto \left\{ \begin{alignedat}{3}
        ...
        \right.
    \end{align}
    where $\Delta_\psi = \abs{\psi_{\mat{s}} - \varphi_{\mat{s}}}$ denotes the difference between the bicycle's heading and the goal given a state $\mat{s}$ and $\sigma_{\text{angle}}^2$ is a postive real constant and the Gaussian density function is normalized to a maximum value of 1.

    The reward function assigns constant reward for terminal states and reward based on the angle between the bicycle's angle towards the goal.
\end{definition}
\subsection{Long-Term predictions}
...
\subsection{Long-Term predictions}
\begin{itemize}
    \item Propagation of stop probabilities
\end{itemize}
