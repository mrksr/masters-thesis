...
This thesis chooses the non-parametric approach of the \emph{particle swarm optimization policy~(PSO-P)} described by \citeauthor{hein_reinforcement_2016} in \cite{hein_reinforcement_2016}.
...
\emph{Particle swarm optimization} is the gradient-free heuristic technique which is used in PSO-P to solve this non-convex optimization problem.
It works by creating a set, or \emph{swarm}, of candidate solutions, or \emph{particles}, in the search space.
...
\begin{definition}
    A \emph{PSO-Run} on a set $\Pe$ of particles in $\R^d$ space is a tuple $(f, \Pe, \R^d, \Pos, \Vel, \Neigh)$ of functions where
    \begin{itemize}
