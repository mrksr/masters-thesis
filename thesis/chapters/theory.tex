...
\begin{definition}[Linear Kernel]
    ...
    \begin{align}
        \K_{\text{linear}}(\mat{x}, \mat{y}) \coloneqq \sigma_f^2 \cdot \mat{x}^T \mat{y} = \sigma_f^2 \left\langle \mat{x}, \mat{y}\right\rangle.
    \end{align}
    The parameter $\sigma_f^2 \in \R_{>0}$ is called the \emph{signal variance}.
\end{definition}
\begin{definition}[White Noise Kernel]
    The \emph{white noise kernel} is defined as
    \begin{align}
        \K_{\text{noise}}(\mat{x}, \mat{y}) \coloneqq \begin{cases}
            \sigma_n^2 & \text{ if $\mat{x} = \mat{y}$} \\
            0 & \text{ otherwise.}
        \end{cases}
    \end{align}
    The parameter $\sigma_n^2 \in \R_{>0}$ is called the \emph{signal variance}.
\end{definition}
