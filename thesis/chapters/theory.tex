...
\begin{align}
    ...
    \Prob{\rv{s_{t+1}} \given \rv{s_t}, \pi} &= \begin{cases}
        \delta\left(\rv{s_t}, \rv{s_{t+1}}\right) & \text{ if $\rv{s_t} \in \Tee$} \\
...
\end{align}
If $f$ is bounded above, this limit is well-defined since $f(\Cog_p(\cdot))$ is monotonically increasing for all $p \in \Pe$.
...
Based on empirical studies, \citeauthor{eberhart_comparing_2000} suggest setting $\omega$ to 0.7291 and both $\gamma_c$ and $\gamma_s$ to 1.49618 \cite{eberhart_comparing_2000,clerc_swarm_1999}.
